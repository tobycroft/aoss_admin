...
use app\aoss\model\AttachmentModel;
...
class Attachment extends Admin
{
     ...
     */
    public array $type = ["all" => "都保存一遍", "local" => "仅本地", "oss" => "仅OSS", "remote" => "存远程"];
    public array $main_type = ["local" => "本地", "oss" => "OSS"];
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (AttachmentModel::update($data)) {
                $this->success('编辑成功');
            } else {
                $this->error('编辑失败');
            }
    ...
    public function quickEdit($record = [])
    {
        ...
        $result = AttachmentModel::where("id", $id)
            ->setField($field, $value);
        if (false !== $result) {
            action_log('Attachment_param_edit', 'Attachment_param', $id, UID);
            $this->success('操作成功');
        } else {
            $this->error('操作失败');
        }
